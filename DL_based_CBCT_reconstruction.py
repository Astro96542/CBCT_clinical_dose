...
class DL_based_CBCT_reconstruction:
    def __init__(self):
        ...
        try:
            ...
            state_dict = torch.load('./pretrained_ckpt/ID_model_ckpt.pth')
            from collections import OrderedDict
            new_state_dict = OrderedDict()
            for k, v in state_dict.items():
                name = k[7:]
                new_state_dict[name] = v
            ID_model.load_state_dict(new_state_dict)
    ...
    def ID_model_processing(self, recon_image):
        ...
        """
        # ndarray splitting, tensor conversion, ID_model processing, & ndarray conversion
        refined_image = np.empty((256, 256, 256), dtype='float32')  # ndarray (256, 256, 256)
        self.ID_model.eval()
        with torch.no_grad():
            recon_image = recon_image.swapaxes(0, 1).swapaxes(1, 2)
            patch_num = 0
            for d in range(2):
                for h in range(2):
                    for w in range(2):
                        recon_slice = recon_image[128 * h:128 * (h + 1), 128 * w:128 * (w + 1), 128 * d:128 * (d + 1)]
                        recon_slice_tensor = transforms(recon_slice)
                        recon_slice_tensor = recon_slice_tensor.reshape(1, 1, 128, 128, 128)
                        recon_slice_tensor = recon_slice_tensor.to(device)
                        output_slice_tensor = self.ID_model(recon_slice_tensor)
                        refined_slice_tensor = recon_slice_tensor + output_slice_tensor
                        refined_slice = torch.squeeze(refined_slice_tensor).cpu().numpy()
                        d_index = patch_num // 4
                        h_w_index = patch_num % 4
                        h_index = h_w_index // 2
                        w_index = h_w_index % 2
                        refined_image[128 * d_index:128 * (d_index + 1), 128 * h_index:128 * (h_index + 1),
                        128 * w_index:128 * (w_index + 1)] = refined_slice
                        patch_num += 1
    ...
    def process(self, clinical_dose_proj):
        ...
        :return: reconstructed 3D image: ndarray (256, 256, 256)
        """
        # Step 1: Perform FDK reconstruction on the de-noised projection data
